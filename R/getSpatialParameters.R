...
find_kernel_outliers_for_sensitivity <- function(pattern, locs, pattern_threshold = 0.15, sigma = 10, kernelthreshold = 2, method = "Pattern_Threshold", outlier = "positive")
{
    ...
    Karr <- vapply(seq(1,100), function(i) {Xr <- X; spatstat.geom::marks(Xr) <- spatstat.geom::marks(X)[pracma::randperm(seq_len(length(spatstat.geom::marks(X))))]; temp <- spatstat.explore::Smooth(Xr, at = "points", sigma = sigma, leaveoneout = TRUE); return(temp)}, numeric(length(Kact)))
    Kvec <- unlist(Karr)
    mKvec <- mean(Kvec)
    sKvec <- sd(Kvec)
...
getOptimalSigmaThresh <- function(pattern, locs, sigmaVec, threshVec){
    ...
    allwin <- spatstat.geom::owin(xrange = c(min(locs$x),max(locs$x)), yrange = c(min(locs$y),max(locs$y)))
    X <-spatstat.geom::ppp(x = locs$x, y = locs$y, window = allwin, marks = pattern)
    Ks <- vapply(sigmaVec,function(i) spatstat.explore::Smooth(X, at = "points", sigma = i, leaveoneout = TRUE), numeric(X$n))
    mor_1 <- vapply(seq_len(length(sigmaVec)), function(i) unlist(ape::Moran.I(spatstat.geom::marks(X)-Ks[,i], visium.dist.inv)),numeric(4))
    ...
    smallKs <- vapply(smallSigmaVec,function(i) spatstat.explore::Smooth(X, at = "points", sigma = i, leaveoneout = TRUE), numeric(X$n))
    smallmor_2 <- vapply(seq_len(length(smallSigmaVec)), function(i) unlist(ape::Moran.I(spatstat.geom::marks(X)-smallKs[,i], visium.dist.inv)), numeric(4))
    ...
    mor_2_ind <- vapply(seq_len(length(threshVec)), function(j){visium.dist.inv_1 <- visium.dist.inv; visium.dist.inv_1[inds2[,j]==0] <- 0; unlist( ape::Moran.I(spatstat.geom::marks(X)-smallKs[,sigmaOpt1_ind],visium.dist.inv_1))}, numeric(4))
...
#' @return a numeric matrix of sigmaOpts - the optimal width of the gaussian distribution, and the thresOpt - outlier threshold around the set of spots for each pattern
#' @examples
#' # Create test data
#' cells <- c()
#' test_num <- 500
...
getSpatialParameters <- function(spatialPatterns){
    ...
    optParams <- vapply(patternList, function(i) unlist(getOptimalSigmaThresh(pattern = spatialPatterns[,i], locs = data.frame(x = spatialPatterns$x, y = spatialPatterns$y), sigmaVec, threshVec)), numeric(2))
    return(optParams)
}
