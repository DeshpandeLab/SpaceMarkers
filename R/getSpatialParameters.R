...
find_kernel_outliers_for_sensitivity <- function(pattern, locs, pattern_threshold = 0.15, sigma = 10, kernelthreshold = 2, method = "Pattern_Threshold", outlier = "positive")
{
  ...
  Kact = spatstat.core::Smooth(X, at = "points", sigma = sigma, leaveoneout = T)
  Karr = sapply(seq(1,100), function(i) {Xr = X; spatstat.geom::marks(Xr) = spatstat.geom::marks(X)[pracma::randperm(1:length(spatstat.geom::marks(X)))]; temp = spatstat.core::Smooth(Xr, at = "points", sigma = sigma, leaveoneout = T); return(temp)})
  Kvec = unlist(Karr)
  mKvec = mean(Kvec)
  sKvec = sd(Kvec)
...
getOptimalSigmaThresh <- function(pattern, locs, sigmaVec, threshVec){
  ...
  allwin <- spatstat.geom::owin(xrange = c(min(locs$x),max(locs$x)), yrange = c(min(locs$y),max(locs$y)))
  X <-spatstat.geom::ppp(x = locs$x, y = locs$y, window = allwin, marks = pattern)
  Ks <- sapply(sigmaVec,function(i) spatstat.core::Smooth(X, at = "points", sigma = i, leaveoneout = T))
  ...
  smallKs <- sapply(smallSigmaVec,function(i) spatstat.core::Smooth(X, at = "points", sigma = i, leaveoneout = T))
