...
.generate_circos_colors <- function(molecules_in_plot_df, # Should be unique molecules: molecule_name, molecule_type
  ...
  # [EDIT v1] Enforce Brewer's minimum of 3 colors to silence warning; subset back to n_cell_types.
  # Reason: RColorBrewer::brewer.pal() warns when n < 3; we request at least 3, then keep only what's needed.
  n_req   <- max(3, min(n_cell_types, pal_ct_max))
  basepal <- RColorBrewer::brewer.pal(n_req, cell_type_palette_name)
  pal_ct  <- basepal[seq_len(min(n_cell_types, length(basepal)))]
...
.draw_circos_plot_elements <- function(bed_data,
                                       ...
                                       default_ligand_color,
                                       default_receptor_color,
                                       link_directional = 1,  # [EDIT v2] NEW: allow undirected links when set to 0
  ...
  if (!is.null(links_data) && nrow(links_data) > 0) {
    ...
    # [EDIT v1] Provide numeric defaults for arrow width/length when NULL to avoid circlize internal math on NULL.
    # Reason: circlize uses `rou2 - arr.length`; NULL here triggers `argument is of length zero`.
    arr_width_val  <- if (is.null(link_arrowhead_width))  0.02 else link_arrowhead_width
    arr_length_val <- if (is.null(link_arrowhead_length)) 0.08 else link_arrowhead_length
    
    for (i in 1:nrow(links_data)) {
      link <- links_data[i, ]
      if (link$source_start >= link$source_end || link$target_start >= link$target_end) next
      if (isTRUE(link_directional == 0)) {
        # UNDIRECTED: do NOT pass arrowhead arguments to avoid internal polygon() path
        ...
        # DIRECTED: include arrowhead parameters
        circlize::circos.link(
          sector.index1 = link$source_cell_type, 
          point1 = c(link$source_start, link$source_end),
          sector.index2 = link$target_cell_type, 
          ...
          border = NA,
          directional = 1,
          arr.type = link_arrowhead_type,
          arr.col = link_cols_vec[i], 
          arr.width = arr_width_val, 
          arr.length = arr_length_val
        )
      }
    }
  }
...
#' @param link_directional Integer 1 or 0. `1` = directed with arrowheads (default);
#'   `0` = undirected (no arrows).
#' @param score_color_palette_fun A function (e.g., from `circlize::colorRamp2`) to map scores to colors.
#'   If NULL, a default continuous palette will be used internally (if implemented downstream).
#' @param split_segments_for_links Logical. If TRUE, molecule segments are sized by interaction
#'   count/score and links connect to unique sub-segments. Default is TRUE.
...
#' @param score_transform_for_width Function to transform scores for width scaling.
#'   Default is `function(s) s`.
#' @param use_individual_molecule_colors Logical. If TRUE, each unique ligand/receptor name
#'   gets a distinct color. Default is TRUE.
#' @param default_ligand_color Character, color for all ligand segments if
#'   `use_individual_molecule_colors` is FALSE. Default is "lightgreen".
#' @param default_receptor_color Character, color for all receptor segments if
#'   `use_individual_molecule_colors` is FALSE. Default is "lightblue".
#' @param individual_ligand_palette_generator Function (takes `n`, returns `n` colors)
#'   for unique ligands. Default generates a green palette internally (if NULL).
#' @param individual_receptor_palette_generator Function (takes `n`, returns `n` colors)
#'   for unique receptors. Default generates a blue palette internally (if NULL).
#' @param molecule_segment_border_col Color for the border of L/R segments.
#'   Default `NA` (no border).
#' @param inter_molecule_segment_gap Numeric, gap between L/R segments on the same track.
#'   Default is 0.1.
#'
...
#'   )
#'
#'   ## Basic directed plot (default)
#'   # plot_cell_interaction_circos(lr_data)
#'
#'   ## Undirected plot (no arrows)
#'   # plot_cell_interaction_circos(lr_data, link_directional = 0)
#'
...
plot_cell_interaction_circos <- function(lr_interactions_df,
                                     ...
                                     link_directional = 1,  # [EDIT v2] NEW: expose in top-level; default keeps current behavior
    ...
    .draw_circos_plot_elements(
        ...
        link_directional = link_directional  # pass through
    )
    return(invisible(NULL))
}
...
#'   If NULL, the source cell is first (if present), followed by target cells
#'   in alphabetical order.
#' @param link_arrowhead_type Character. Type of arrowhead (e.g., "triangle",
#'   "big.arrow"). Default is `"triangle"` for this source-to-target view.
#'
...
#'
#'   lr_data <- data.frame(
#'     ligand = c("LGF1", "LGF1", "LGF2", "LGF3", "LGF4",
#'                "LGF1", "LGF5", "LGF6", "LGF7"),
#'     receptor = c("REC1", "REC2A", "REC1", "REC3B", "REC1",
#'                  "REC4", "REC4", "REC2A", "REC5"),
#'     source_cell_type = c("CellA", "CellA", "CellB", "CellC",
...
plot_source_to_target_circos <- function(lr_interactions_df,
  ...
  .draw_circos_plot_elements(
    ...
    use_individual_molecule_colors=use_individual_molecule_colors,
    default_ligand_color=default_ligand_color, default_receptor_color=default_receptor_color,
    link_directional = link_directional
  )
  return(invisible(NULL))
}
...
#' @param cell_order Optional character vector for specific cell type ordering.
#'   If NULL, the target cell is first, then sources alphabetically.
#'
#' @return Invisibly returns NULL. The function is called for its side effect of creating a plot.
#'
#' @export
...
#'   #   lr_interactions_df = test_lr_data,
#'   #   source_cell_names   = c("CellB", "CellD"),
#'   #   target_cell_name    = "CellA"
#'   # )
#'
#'   ## Interactions from CellA and CellC targeting CellD with score-scaled widths
#'   # plot_target_from_sources_circos(
#'   #   lr_interactions_df        = test_lr_data,
