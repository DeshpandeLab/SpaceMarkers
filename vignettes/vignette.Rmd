...
```{r}
system(paste0("wget -q ",cogaps_url, " -O CogapsResult_5.rds"))
...
SpaceMarkers <- getInteractingGenes(data = counts_matrix, reconstruction = cogaps_matrix, optParams = optParams_breast_cancer,spatialPatterns = spPatterns, refPattern = SpaceMarkersRefPattern, mode = SpaceMarkersMode)
```
...
SpaceMarkersMode = "DE"
SpaceMarkers_DE <- getInteractingGenes(data = counts_matrix, reconstruction = NULL, optParams = optParams_breast_cancer,spatialPatterns = spPatterns, refPattern = SpaceMarkersRefPattern, mode = SpaceMarkersMode)
...
for (g in 1:length(interactions)){
  ...
  print(paste0("Top 3 genes for ",interactions[g]))
  print(df$Gene[1:3])
  gene_list <- c(gene_list,df$Gene[1:3])
...
for (ii in 1:length(gene_list)){
  ...
  plist[[ii]] <- mplot2 %>% ggplot( aes_string(x='region', y=gene_list[ii], fill='region')) +
    geom_boxplot() +
     ...
     ggtitle(paste0(gene_list[ii]," Expression (Log)")) +
    xlab("") 
}
```
...
```{r}
# Table of statistics for this pattern
```
...
```{r}
#Table of Statistics for this Pattern Interaction
...
```{r}
#Table of Interaction for this Pattern
```
...
```{r}
# marker_df <- SpaceMarkers$interacting_genes[[3]]
# marker_df <- marker_df[marker_df$`Pattern_1 x Pattern_5` == "vsBoth",]
# #marker_df <- marker_df[marker_df$`Pattern_1 x Pattern_5` == "vsBoth",]
# gene_list = marker_df[,"KW.pvalue"]
# names(gene_list) = marker_df[,"Gene"]
# gene_list = sort(gene_list, decreasing = TRUE)
# mysymbols <- names(gene_list)
...
```{r}
set.seed(7)
# Use metric: (1 - p) * (1 - p) * (1 - p) - for KW, Dunn1 and Dun2
marker_df2 <- SpaceMarkers$interacting_genes[[4]]
marker_df2$metric <-  (1 - marker_df2$KW.p.adj) * (1 - marker_df2$Dunn.pval_1_Int.adj) * (1 - marker_df2$Dunn.pval_2_Int.adj)
#marker_df2 <- marker_df2[marker_df2$`Pattern_1 x Pattern_5` == "vsBoth",]
matx2 <- AnnotationDbi::select(org.Hs.eg.db, keys=marker_df2[,"Gene"], columns=c("ENTREZID","GENENAME"), keytype="ALIAS")
etz_df <- merge(x=matx2,y=marker_df2, by.x="ALIAS", by.y="Gene",all.x=TRUE)
etz_list = etz_df[,"metric"]
names(etz_list) = etz_df[,"ENTREZID"]
#etz_list <- etz_list[complete.cases(names(etz_list))]
#etz_list <- -log10(etz_list)
#etz_list <- (etz_list-mean(etz_list))/sd(etz_list)
etz_list <- etz_list[order(-etz_list)]
#etz_list <- etz_list[!duplicated(names(etz_list))]
#edo2 <- gseDO(etz_list)
edo2 <- gseDO(etz_list, score = "pos")
```


## Enrichment Plots
```{r}
dotplot(edo2, x = 'Count', font.size = 8)
ridgeplot(edo2) + theme(axis.text.y = element_text(size = 8))
```
