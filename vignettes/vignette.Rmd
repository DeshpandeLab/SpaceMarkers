...
getInteractingGenes also takes several minutes on a MacBook Pro with an Intel Core i5 processor and 16 GB of memory, so for both residual and DE mode, the saved data from a previous run will be used.

```{r eval = FALSE}
SpaceMarkers <- getInteractingGenes(data = counts_matrix, reconstruction = cogaps_matrix, optParams = optParams,spatialPatterns = spPatterns, refPattern = SpaceMarkersRefPattern, mode = SpaceMarkersMode, analysis = "overlap")
```
...
For the 'interacting_genes' data frames, the first column is the list of genes and the second column says whether the expression of genes are statistically significant when compared to the refPattern only, the interacting pattern only, or both. The remaining columns are statistics for the Kruskal-Wallis test and the post hoc Dunn's test. The SpaceMarkersMetric column is a product of the Dunn's statistics and is used to rank the genes.
...
```{r eval = FALSE}
SpaceMarkersMode = "DE"
SpaceMarkers_DE <- getInteractingGenes(data = counts_matrix, reconstruction = NULL, optParams = optParams,spatialPatterns = spPatterns, refPattern = SpaceMarkersRefPattern, mode = SpaceMarkersMode, analysis = "overlap")
```
```{r}
data("SpaceMarkers_DEmode_overlap")
SpaceMarkers_DE <- SpaceMarkers_DEmode_overlap
```
...
Here we identify the top 20 genes in 'residual' mode and their corresponding ranking in DE mode. APOC1,IGHE,IFI30,CLU,CST1,CAPG,TGM2,TREM2 and GCHFR  are ranked high in both 'DE' and 'residual' mode. While 2 genes that are ranked high in 'residual' mode are not identified at all in 'DE' mode
...
There were 6 genes that were interacting in 'DE' mode but not 'residual' mode. Recall that DE mode looks at the information encoded in the latent feature space and does not filter out genes based on any confounders between the counts matrix and latent feature matrix as is done in 'residual' mode.
...
One new feature of the SpaceMarkers package is the type of analysis that can be carried out, whether 'overlap' or 'enrichment' mode. The major difference between the two is that enrichment mode includes genes even if they did not pass the multi-test correction for the KW test. These additional genes were included to enable a more statistically powerful pathway enrichment analysis and understand to a better extent the impact of genes involved each pathway. 

### Residual Mode and DE Mode - Enrichment 
```{r eval = FALSE}
SpaceMarkersMode = "residual"
SpaceMarkers_DE <- getInteractingGenes(data = counts_matrix, reconstruction = NULL, optParams = optParams,spatialPatterns = spPatterns, refPattern = SpaceMarkersRefPattern, mode = SpaceMarkersMode, analysis = "enrichment")
```

```{r}
data("SpaceMarkers_Residualmode_enrichment")
SpaceMarkers_enrich <- SpaceMarkers_Residualmode_enrichment
```
...
```{r}
data("SpaceMarkers_DEmode_enrichment")
SpaceMarkers_DE_enrich <- SpaceMarkers_DEmode_enrichment
```

#### Comparing Enrichment Analyses

```{r}
residual_p1_p2_enrich <- SpaceMarkers_enrich$interacting_genes[[4]]
DE_p1_p2_enrich <- SpaceMarkers_DE_enrich$interacting_genes[[4]]

```

```{r}
enrich_res_to_de <- compare_genes( head(residual_p1_p2_enrich$Gene, n = 20) ,DE_p1_p2_enrich$Gene,ref_name = "residual_Enrich",list2_name = "DE_Enrich" )
enrich_res_to_de
```
As seen before there are significant differences between DE and residual mode with how the genes are ordered. However, both methods are ranking the few genes highly possibly due to greater biological importance.

```{r}
overlap_enrich_de <- compare_genes( head(DE_p1_p2$Gene, n = 20) ,DE_p1_p2_enrich$Gene,ref_name = "DE_Enrich",list2_name = "DE_Overlap" )
overlap_enrich_de
```
The enrichment analysis and overlap analysis are in great agreement, with only a few small changes. 
```{r}
overlap_enrich_de_later <- compare_genes( DE_p1_p2$Gene ,DE_p1_p2_enrich$Gene,ref_name = "DE_Enrich",list2_name = "DE_Overlap", sub_slice = 30:50)
overlap_enrich_de_later
```
Further down the gene list, the ranking begins to shuffle alot more.Since the SpaceMarkersMetric only takes into account the Dunn's test, the genes, genes that may not have passed the correction for the KW test inititally are now taken into account with great differences between any two patterns. 
...
In this particular analysis we will use the 'enrichment' genes for residual mode as a background and the 'overlap' genes to search for enrichment
```{r}

matx2 <- AnnotationDbi::select(org.Hs.eg.db, keys=residual_p1_p2_enrich[,"Gene"], columns=c("ENTREZID","GENENAME"), keytype="ALIAS")
etz_df <- merge(x=matx2[-which(duplicated(matx2$ALIAS)),],y=residual_p1_p2_enrich, by.x="ALIAS", by.y="Gene",all.x=TRUE)
...
#Regular Gene list
target_genes <- AnnotationDbi::select(org.Hs.eg.db, keys=residual_p1_p2[,"Gene"], columns=c("ENTREZID","GENENAME"), keytype="ALIAS")
...
The spatial heatmap for IFI30 shows fairly high expression in multiple patterns but especially in the interacting regions. This is reflected in the box plots as well with fairly high expression in the interacting pattern vs the other two patterns.
...
Unlike the previous three genes the KW pvalues and Dunn's pvalues are relatively high so the distinction will not appear as pronounced. The ING1 gene shows relatively high expression in Patterns_5 and the interacting pattern vs Pattern_1. This demonstrates genes in a `Pattern_n X Pattern_n` can show up on the top of the list by this SpaceMarkersMetric as long as there is a significant difference regardless of the direction of the difference between the patterns.
...
In this comparison the box plots show high expression of TMEM147 in Pattern_5 and the interacting region compared to Pattern_1. However, the prevalent expression profile for RCN1 is harder to distinguish. Like the Pattern_1 Only vs Pattern_5, the p-values here are relatively high indicating low statistical significance.
