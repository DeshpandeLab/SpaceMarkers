...
Directed analyses use a richer four- or five-way classification: each spot
is labeled as `Pattern_1`, `"Pattern_1 near Pattern_5"`, `"Pattern_5 near
Pattern_1"`, `Pattern_5`, or `"both"` (the doubly-hot confounder region).
`plot_spatial(..., source = "interaction")` auto-picks the directed scheme
when pattern + influence hotspots are both populated, and delegates to
`overlap_map()` so the plot categories match anything the user derives
directly:

```{r}
labs <- overlap_map(sme_directed, c("Pattern_1", "Pattern_5"))
