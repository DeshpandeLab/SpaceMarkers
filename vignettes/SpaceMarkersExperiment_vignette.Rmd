...
## Directed interaction overlay

For the chosen pair, the spot-level classification is richer than the
undirected one: each spot is labeled `Pattern_1`, `"Pattern_1 near
Pattern_5"`, `"Pattern_5 near Pattern_1"`, `Pattern_5`, or `"both"` (the
doubly-hot confounder region). `plot_spatial(..., source =
"interaction")` auto-picks the directed scheme when pattern + influence
hotspots are both populated, and delegates to `overlap_map()` so the
plot categories match anything the user derives directly:
