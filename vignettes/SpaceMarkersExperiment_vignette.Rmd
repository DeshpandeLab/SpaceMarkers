...
Since the SME holds all data, visualization functions can accept the object
directly or you can extract data with accessors.

## IM Scores Bar Plot

```{r, message=FALSE, warning=FALSE, dpi=60, fig.width=6}
plot_im_scores(sme_undirected, interaction = "Pattern_1_Pattern_2", nGenes = 10)
```

## Overlap Scores

```{r}
calculate_overlap_undirected(hotspots(sme_undirected, type = "undirected"))
```

## Spatial Plots

`plot_spatial()` is the SME-native plotting bridge. It automatically extracts
coordinates from the object, finds the tissue image stored in the object's
`visiumDir`, and falls back to a blank background if no image is available.
Features are looked up in order: `colData` (patterns, influence values),
`rownames` (gene expression), or hotspot metadata.
...
`plot_spatial()` accepts gene names directly — it looks them up in the
expression matrix automatically, so no manual data extraction is needed:

```{r, message=FALSE, warning=FALSE, dpi=60, fig.width=6}
ims <- undirected_scores(sme_undirected)
top_genes <- ims[order(ims$Pattern_1_Pattern_2, decreasing = TRUE), "Gene"]
