...
myPalette <- function(numLevels) {
    return(colorRampPalette(c("white","blue"))(numLevels))}
```
...
```{r, message=FALSE, warning=FALSE}
plot_grid(plotlist = list(plist[[1]],plots[[1]]), scale = 0.5, rel_heights = c(4,1))
plot_grid(plotlist = list(plist[[2]],plots[[2]]), scale = 1)
```
...
```{r}
plot_grid(plotlist = list(plist[[3]],plots[[3]]), scale = 0.5)
```
