...
genes is labelled as FALSE. Here is an example of the statistics for such genes. 
```{r}
...
However, from the box plots, TMEM147 and GLB1 show relatively high expression in
Patterns_5 and the interacting pattern vs Pattern_1 only.
...
p-value are high and the SpaceMarkersMetric relatively low. This lack of 
distinction highlights the fact that SpaceMarkers performs best for identifying
interactions for two latent individual latent Patterns interacting with each
other. In other words, the vsBoth genes are ranked highest in the list while
the vsPattern_1 and vsPattern_5 genes which look at an individual Pattern
interaction with another already interacting region are ranked lower in the
SpaceMarkers list.
